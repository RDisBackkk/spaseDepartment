...
export const ProgressSlider: FC<ProgressSliderProps> = ({
  ...
  useEffect(() => {
    ...
    if (getChildren) {
      const el = getChildren as React.ReactElement<{ children: React.ReactNode }>;
      const values = React.Children.toArray(el.props.children).map(
...
export const SliderBtn: FC<SliderBtnProps> = ({
  ...
  return (
    <button
      className={cn(
        `relative ${active === value ? 'opacity-100' : 'opacity-60'}`,
        className
      )}
